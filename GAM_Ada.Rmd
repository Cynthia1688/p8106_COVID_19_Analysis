...
train_data <- train_data |>
 mutate(across(c(gender,diabetes, hypertension,vaccine,severity,race,smoking), ~factor(.)))
test_data <- test_data  |>
  mutate(across(c(gender,diabetes, hypertension,vaccine,severity,race,smoking), ~factor(.)))
...
```{r}
set.seed(2716)
lasso_model =  train(recovery_time~ .,  method = "glmnet",
                     data = train_data,
...
```{r}
set.seed(2716)
enet_model = train(recovery_time~ .,
                   data = train_data,
                   method = "glmnet",
